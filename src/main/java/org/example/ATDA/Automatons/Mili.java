...
import org.example.ATDA.Tables.Generators.TableMilFactory;
...
public class Mili{
    ...
    private OutputTableMil outputsTable;
    private Status status;

    public Mili() {
        this.statuses = StatusFactory.createTable();
        this.outputsTable = TableMilFactory.createTable();
        status = Status.A1;
    }
    ...
    public Output[] bip(Input[] inputs){
        ...
        for(int i = 0; i < inputs.length; i++){
            outputs[i] = outputsTable.getOutput(status, inputs[i]);
    ...
    public Output bip(Input input){
        Output output;
            ...
            output = outputsTable.getOutput(status, input);
